...
3. We also have a working code that uses the EDGAR SDK and EDGAR API to fetch live numeric/metric data, that gets into structured financial data.
   - This exists as an independent pipeline or module, apart from text crawling or huggingface data. This work module has its own DAG, its test suite.
...
#### DAG 1 - SEC Filings Extraction.
1. One of the Airflow DAGs designed automates the ETL workflow for SEC filings - from fetching company data to generating structured outputs in a parquet file. This DAG also integrates its last step with the AWS ETL pipeline (Merge Pipeline) for seamless data merging and versioning. This establishes the consolidated fact data table.
...
#### DAG 2 - Financial Data Extraction (Metrics) 
1. Another Airflow DAG that we have is the one which uses the EDGAR SDK/API, which obtains the XBRL data. The financial data that comes from the official source is authentic and established factual data. (`https://www.sec.gov/search-filings/edgar-application-programming-interfaces`) [In progress.]
